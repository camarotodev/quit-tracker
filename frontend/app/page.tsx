...
export default function Home() {
  ...
  function calcularTempo(startDate: string) {
    ...
    if (dias > 0) return `${dias} dias ${horas}h ${minutos}min ${segundos}s`
  ...
  async function resetarTempo(id: number) {
    await fetch(`http://localhost:8080/habits/${id}/reset`, {
      method: "PATCH",
    })
  ...
  return (
    <div className="flex flex-col justify-center items-center font-bold text-2xl mt-6">
      ...
      <div className="mt-5 flex flex-col text-center">
        ...
        <div>
          {vicios.map((vicio: any) => (
            <div className="bg-zinc-800 rounded-lg mt-6 p-1 pl-10 pr-10 w-auto" key={vicio.id}>
              <h2 className="mb-3 mt-2">{vicio.name}</h2>
              <p className="text-zinc-400 text-xs">Tempo de abstinência</p>
              <p className="text-lg">
                {calcularTempo(vicio.startDate)}
              </p>

              <button
                className="cursor-pointer mt-3 mb-3 text-xs text-zinc-400 hover:text-red-400 transition-colors"
                onClick={() => resetarTempo(vicio.id)}
              >
                🔄 Resetar tempo
              </button>
            </div>
      ...
      <h1 className="mx-5 mt-5 mb-2">Adicionar vício</h1>
